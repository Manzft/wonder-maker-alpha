...
func _physics_process(delta):
	if (get_node("../Editor").playing):
		...
		if (active && !exiting):
			...
			#Carrying
			if (get_node("../Character").is_on_floor() && canPick && visible && currentSprite.animation != "pressed" && press && !carrying && get_node("../Character").running && !get_node("../Character").carrying && !get_node("../Character").sneaking):
				carrying = true;
				canPick = false;
				canPickTimer = 0.0;
