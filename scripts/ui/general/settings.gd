...
func _process(delta):
	...
	if (round(Global.ENTITY_PHYSICS_SPEED) == 100):
		$PhysicsInterpolation.disabled = true
		Global.PHYSICS_INTERPOLATION = false
		$PhysicsInterpolation.modulate = Color("#b0b0b0")
	else:
		$PhysicsInterpolation.modulate = Color("#ffffff")
		$PhysicsInterpolation.disabled = false
